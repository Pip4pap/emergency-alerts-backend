...
module.exports = {
  ...
  getMyHospital: catchAsync(async (req, res, next) => {
    ...
    res.status(200).json({
      status: 'success',
      data: hospitalAdminHospital,
    });
  }),
  addMetoHospital: catchAsync(async (req, res, next) => {
    let hospital = await Hospital.findOne({ where: { hospitalName: req.body.hospitalName } });
  ...
  approveHospitalAdmin: catchAsync(async (req, res, next) => {
    let hospitalAdmin = Hospital.findOne({ where: { email: req.params.email } });

    res.status(200).json({
      status: 'success',
      message: 'Successfully approved admin to Hospital',
    });
  }),
