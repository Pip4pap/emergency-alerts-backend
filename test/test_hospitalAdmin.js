...
describe('Hospital Admin', () => {
  ...
  describe('POST /api/hospitalAdmin/login', () => {
    it('It should successfully login HospitalAdmin with correct credentials', async () => {
      ...
      res.body.data.should.have.property('ID').eq(`${hospitalAdmin.ID}`);
      res.body.data.should.have.property('email').eq(`${hospitalAdmin.email}`);
      res.body.data.should.have.property('role').eq(`${hospitalAdmin.role}`);
    });
  ...
  describe('POST /api/hospitalAdmin/signup', () => {
    it('It should successfully add HospitalAdmin to db and Log them in', async () => {
      ...
      const requestPromise = new Promise((resolve, reject) => {
        client
          .post('/api/hospitalAdmin/signup')
          .send(generators.newHospitalAdmin)
          .then((res) => {
            resolve(res);
          });
      });
  ...
  describe('/api/hospitalAdmin/:id/hospital', () => {
    describe('GET /api/hospitalAdmin/:id/hospital', () => {
      it("It should return the HospitalAdmin's hospital", async () => {
        ...
        const { ID, email, password } = generators.newHospitalAdmin;
        const loginResponse = await login(email, password);
        const { token } = loginResponse.body;

        const requestPromise = new Promise((resolve, reject) => {
          client
            .get(`/api/hospitalAdmin/${ID}/hospital`)
